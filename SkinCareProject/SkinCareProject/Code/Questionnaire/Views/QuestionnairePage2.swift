...
struct QuestionnairePage2: View {
    ...
    var body: some View {
        VStack {
            ...
            //botão carrega user info ate pagina final e ali faz update
            //n pode ter navigationlink
            Button(action: { vm.updateUser(publicDb: false, appUser: vm.user[0], recordType: .User, userVegan: vm.user[0].vegan, userSkinType: buttonPressed)}, label: { Text("button")})
            
            HStack(alignment: .center) {
//                if buttonLabel == .next {
//                    Button(action: { vm.updateUser(publicDb: false, appUser: vm.user[0], recordType: .User, userVegan: vm.user[0].vegan, userSkinType: buttonPressed)}, label: Text("button"))
//                    Button(action: { vm.updateUser(publicDb: false, appUser: vm.user[0], recordType: .User, userVegan: vm.user[0].vegan, userSkinType: buttonPressed)}) {
//
